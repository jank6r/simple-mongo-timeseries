...
function mongoTSAggregation(config) {
    ...
    var allocateAggragation = function (key, value) {
        ...
        for (i = 0; i < 24; i++) {
            ...
            for (j = 0; j < 6; j++) {
                allocationObj.detail.push({id: Number(j), value: null, min: null, max: null});
            }
        ...
        config.getCollection().insertOne(updateData, function (err) {
            if (err) {
                deferred.reject(new Error(err));
            } else {
                deferred.resolve(key);
                ...
                if (err) {
                    ...
                    if (result.length === 0) {
                        ...
                        result.forEach(function (item) {
                            ...
                            // workaround type convertion
                            item.detail = item.detail.map(function(d) {
                                d.id = parseInt(d.id);
                                return d;
                            });
