...
function mongoTSRead(config) {
    ...
    var selectAggregationByHour = function (sensor, fromDate, toDate) {
        ...
        var lowerLimit = moment(fromDate).format('YYMMDDHH');
        var upperLimit = moment((toDate || new Date())).format('YYMMDDHH');
        ...
        config.getCollection().aggregate(
            [
                ...
                {$unwind: '$hours'},
                {
                    $project: {
                        _id: 1,
                        order: {$concat: [{'$substr': ['$$CURRENT._id', sensor.length + 3, -1]},
                            {'$substr': [{$add: ['$$CURRENT.hours.hour', 100]}, 1, 3]}]},
                ...
                {$match: {value: {$not: {$type: 10}}}},
                {
                    $match: {
                        $and: [
                            {order: {$gte: lowerLimit}},
                            {order: {$lte: upperLimit}}
                        ]
                    }
                },
                {$sort: {_id: 1, 'hour': 1}}
            ...
            [
                ...
                {$unwind: '$hourDetail'},
                {
                    $project: {
                        _id: 1,
                        order: {$concat: [{'$substr': ['$$CURRENT._id', sensor.length + 3, -1]},
                            {'$substr': [{$add: ['$$CURRENT.hour', 100]}, 1, 3]}]},
                        hour: 1,
