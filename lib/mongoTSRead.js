...
function mongoTSRead(config) {
    ...
    var buildAggregationQuery = function (fromKeyBuilder, toKeyBuildere) {
    ...
    var selectAggregationByHour = function (sensor, fromDate, toDate) {
        ...
        var lowerKey = fromKeyBuilder.buildAggregationString();
        var upperKey = toKeyBuilder.buildAggregationString();
                        ...
                        $and: [
                            {
                                $or: [
                                    {_id: {$gt: lowerKey}},
                                    {$and: [{_id: {$gte: lowerKey}}, {hour: {$gte: fromKeyBuilder.getHour()}}]}
                                ]
                            },
                            {
                                $or: [
                                    {_id: {$lt: upperKey}},
                                    {$and: [{_id: {$lte: upperKey}}, {hour: {$lte: toKeyBuilder.getHour()}}]}
                                ]
                            }
                        ]
                    }
